...
cam_back.BETAL.img = 0;
cam_back.CEGAIN.img = 0;
cam_back.CELOSS.img = 0;
cam_back.CNEAR.img = 0;
...
counter = 0;
fig_1 = figure(1);
set(fig_1, 'position', [0, 910, 1193, 930]);
set(fig_1, 'color', 'w');
fig_2 = figure(2);
set(fig_2, 'position', [0, 70, 1193, 750]);
set(fig_2, 'color', 'w');
while 1
    % Reset stip plot
    if lcaGetSmart('SIOC:SYS1:ML01:AO053')==1
        PYRO = [];
        ex_charge = [];
        gamma_yield = [];
        gamma_max = [];
        lcaPutSmart('SIOC:SYS1:ML01:AO053', 0);
    end

    % Take new background images
    if lcaGetSmart('SIOC:SYS1:ML01:AO052')==1
        ...
        lcaPutSmart('SIOC:SYS1:ML01:AO052', 0);
    end
...
%             cmax = max(BETAL.img((xx.^2+yy.^2)<30^2));
            ...
            axes('position', [0.82, 0.1, 0.1, 0.8])
...
%             title('CNEAR (log scale)');
            figure(2)
            axes('position', [0.05, 0.05, 0.2, 0.4])
...
%             hold on;
            axes('position', [0.05, 0.55, 0.2, 0.4])
            ...
            ylim([0 3000]);
            legend('Peak count', 'Total count (MC)');
            title('Gamma-rays'); 
...
%             set(fig_cnear,'CData',log10(fliplr(flipud(CNEAR.img))'));
            ...
            if length(gamma_yield)>1000
                set(fig_gamma2(1),'XData', 1:1000, 'YData', gamma_yield(end-999:end)/1e5);
                set(fig_gamma2(2),'XData', 1:1000, 'YData', gamma_max(end-999:end));
            else
                set(fig_gamma2(1),'XData', 1:length(gamma_yield), 'YData', gamma_yield/1e5);
...
%             set(fig_bar,'YData',[PYRO/1e3, ex_charge*1.6e-7, BETAL.gamma_yield/1e7]);
    ...
    % Print2elog
    if lcaGetSmart('SIOC:SYS1:ML01:AO051')==1
        set(fig_1, 'PaperPosition', [0.25, 2.5, 12, 9]);
        util_printLog(fig_1);
        lcaPutSmart('SIOC:SYS1:ML01:AO051', 0);
    end
    if lcaGetSmart('SIOC:SYS1:ML01:AO054')==1
        set(fig_2, 'PaperPosition', [0.25, 2.5, 12, 9]);
        util_printLog(fig_2);
        lcaPutSmart('SIOC:SYS1:ML01:AO054', 0);
    end
    counter = counter + 1; 
    pause(0.1);
end
