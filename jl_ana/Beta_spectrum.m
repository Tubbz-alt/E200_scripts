function [d]= Beta_spectrum(Lanex_ref, Nist_ref,pick,P,do_15mm)
...
%%% - "do_15mm" replaces the integrated spectrum on all angles by a
%%% spectrum  evaluated on a ring with a radius of 15mm. 
%%% 
...
if do_15mm ==0
    if P==2
        ref=load([path_betaspect 'Betatron_spectrum_2.txt']);
    elseif P==4
        ref=load([path_betaspect 'Betatron_spectrum_4.txt']);
    elseif P==8
        ref=load([path_betaspect 'Betatron_spectrum_8.txt']);   
    elseif P==16
        ref=load([path_betaspect 'Betatron_spectrum_16.txt']);
    elseif P==32
        ref=load([path_betaspect 'Betatron_spectrum_32.txt']);
    else    
        error('Invalid Pressure')
    end
else
    if P==2
        ref=load([path_betaspect 'Betatron_spectrum_2_15mm.txt']);
    elseif P==4
        ref=load([path_betaspect 'Betatron_spectrum_4_15mm.txt']);
    elseif P==8
        ref=load([path_betaspect 'Betatron_spectrum_8_15mm.txt']);   
    elseif P==16
        ref=load([path_betaspect 'Betatron_spectrum_16_15mm.txt']);
    elseif P==32
        ref=load([path_betaspect 'Betatron_spectrum_32_15mm.txt']);
    else    
        error('Invalid Pressure')
    end
end
