...
%%%  the measured signal on the different filters, the simulated signal at a given energy,
%%%  and the relative errors between simulation measurement. 
...
%% Load useful variabless 
load('./K_B1L_10567.mat');
load('./K_B2L_10567.mat');
load('./S_num.mat');
...
if do_shot_pick;
       ...
       elseif strcmp(data_set, 'E200_10756')
          I(1:11)=1; J=[4,7, 8, 10, 12, 13, 14, 15, 16, 18, 19];  
...
processed.scalars.ENERGY_FIT_Err_max=zeros(n_step,n_shot);
...
if do_fit==1 
           ...
           %pick= [1:2 4:7 9:14]; 
          % pick=[2:5,7,9:12,14]
           [ec_fit, d_min]=fminsearch(@(z) energy_fit(z, S_BETA1,S_BETA2, S_BETAL,Synchro_ref, Lanex_ref, Nist_ref, pick, K_B1L, K_B2L),10);
...
%%
 ...
 h_text2 = axes('Position', [0.58, 0.95, 0.2, 0.05], 'Visible', 'off');
        ...
        if ~is_qs_scan && ~is_qsbend_scan; 
            QS_text = text(1., 0.5, ['QS = ' num2str(QS, '%.0f') ' GeV'], 'fontsize', 20);
...
%% Figure settings 
 ...
 ax_S_ratio1Lmes = axes ('position', [0.05, 0.32, 0.26, 0.25]);
        bar( S_ratio1L, 'b');
        fig_S_ratio1Lmes = get(gca, 'Children');
        name= {'Cu 1' ; 'Cu 3' ; 'Cu 10' ; 'W 3' ;'W 1' ; 'Cu 0.3'; 'No filter'};
        set(gca, 'xticklabel', name);
        title('S BETA1/S BETAL mes');
              
ax_S_ratio1Lsim = axes('position', [0.36, 0.32, 0.27, 0.25]);
        bar(S_num(ec,1:7), 'w');
        fig_S_ratio1Lsim= get(gca, 'Children');
        %name= {'Cu 1' ; 'Cu 3' ; 'Cu 10' ; 'W 3' ;'W 1' ; 'Cu 0.3'; 'No filter'};
        set(gca, 'xticklabel', name);
        title('S BETA1/S BETAL sim');
        
 
  ax_fit_err1L  = axes('position', [0.68, 0.32, 0.27, 0.25]);
        bar(Err_1L(ec,:),'r');
 ...
 ax_S_ratio2Lmes =  axes ('position', [0.05, 0.02, 0.26, 0.25]);
        bar(S_ratio2L,'b');
        fig_S_ratio2Lmes=get(gca, 'Children');
        set(gca, 'xticklabel', name); 
        title('S BETA2/S BETAL mes');

 ax_S_ratio2Lsim = axes('position', [0.36, 0.02, 0.27, 0.25]);
        bar(S_num(ec,8:14),'w');
        fig_S_ratio2Lsim=get(gca, 'Children');
        set(gca, 'xticklabel', name); 
        title('S BETA2/S BETAL sim');

ax_fit_err2L = axes('position', [0.68, 0.02, 0.27, 0.25]);
        bar(Err_2L(ec,:),'r');
...
%%  Figure update
       ...
       if ec==ec0 && l~=l0
            ...
            set(fig_S_ratio2Lmes,'YData',S_ratio2L);
            set(fig_S_ratio1Lmes, 'YData', S_ratio1L);
          ...
          if ~do_fit;        set(fig_fit_err1L, 'YData', Err_1L(ec,:));
                             ...
                             set(fig_S_ratio1Lsim,'YData', S_num(ec,1:7));
                             set(fig_S_ratio2Lsim,'YData', S_num(ec,8:14));
                             set(Ec_text, 'string', ['Ec = ' num2str(y(ec))]) ; 
                             pause(0.1);   
          end
          
          if do_plot      figure(2);
                          set(fig_fit_Err,'YData', Err); 
                          end;
    ...
    if do_plot;
            ...
            processed.scalars.ENERGY_FIT_Err_max(I(l),J(l))=err(2);
             ...
             if l==l0   
                        ...
                        y2=line([1 y(size(y,2))], [1.2*d_min 1.2*d_min]);
                       ...
                       set(y2, 'Ydata', [1.2*d_min 1.2*d_min]);
                       set(Ec_fit_text2, 'string', ['Critical Energy = ' num2str(ec_fit)]);
...
%% In case of fit, displays the relative error bars at the fitted critical energy 
        ...
        if do_fit
            figure(1)
            set(fig_fit_err1L, 'YData', interp1(y,Err_1L(1:size(y,2),:),ec_fit));
            set(fig_fit_err2L, 'YData', interp1(y,Err_2L(1:size(y,2),:),ec_fit));
            set(fig_S_ratio1Lsim, 'YData', interp1(y,S_num(1:size(y,2),1:7),ec_fit));
            set(fig_S_ratio2Lsim, 'YData', interp1(y,S_num(1:size(y,2), 8:14),ec_fit));
        end
