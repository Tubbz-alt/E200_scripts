...
charge_calib = im_struct.ana.charge_calib; % CMOS Camera charge calibration, in number of e-/e+ per count
...
im_struct.ana.energy_spectrum(1,:) = charge_calib * 1.6e-7 * im_struct.ana.y_profs(1,:)/(E(2)-E(1)); % Energy spectrum in pC/GeV
im_struct.ana.energy_spectrum_2(1,:) = charge_calib * 1.6e-7 * im_struct.ana.y_profs_small_box(1,:)/(E(2)-E(1)); % Energy spectrum in pC/GeV with transverse integration over small box
for i=2:(size(im_struct.ana.y_profs,1)-1)
im_struct.ana.energy_spectrum(i,:) = charge_calib * 1.6e-7 * 2 * im_struct.ana.y_profs(i,:)/(E(i+1)-E(i-1)); % Energy spectrum in pC/GeV
im_struct.ana.energy_spectrum_2(i,:) = charge_calib * 1.6e-7 * 2 * im_struct.ana.y_profs_small_box(i,:)/(E(i+1)-E(i-1)); % Energy spectrum in pC/GeV with transverse integration over small box
end
im_struct.ana.energy_spectrum(end,:) = charge_calib * 1.6e-7 * im_struct.ana.y_profs(end,:)/(E(end)-E(end-1)); % Energy spectrum in pC/GeV
im_struct.ana.energy_spectrum_2(end,:) = charge_calib * 1.6e-7 * im_struct.ana.y_profs_small_box(end,:)/(E(end)-E(end-1)); % Energy spectrum in pC/GeV with transverse integration over small box
