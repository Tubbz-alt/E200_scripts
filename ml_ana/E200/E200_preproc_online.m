%% E200_preproc_online
%  Function to preprocess data in a generic way online.
%
%  Order of operations:
%  1. get analysis ROIs
%  2. get energy axis
...
%
% M.Litos 11/4/2013
function [ data ] = E200_preproc_online( data, cam_name, readwrite )
...
%% preload all images from the camera
[CAMERA_imgs,CAMERA_bkgs]=E200_load_images(CAMERA,CAMERA.UID);
...
for ishot=1:nshot
    ...
    if ~(CAMERA_preprocessed(ishot)) || strcmpi(readwrite,'overwrite')
        ...
        disp(' get raw image');
        % convert cell arrays
%         CAMERA_img = double(cell2mat(CAMERA_imgs{ishot}));
%         CAMERA_bkg = double(cell2mat(CAMERA_bkgs{ishot}));
        CAMERA_img = double(CAMERA_imgs{ishot});
        CAMERA_bkg = double(CAMERA_bkgs{ishot});
        ...
        % medfilt2 not available on facet-srv :(
